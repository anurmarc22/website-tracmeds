...
export default function PaywallModal({ visible, onClose, onUnlocked }: Props) {
  ...
  const handlePurchase = async (pkg: SubscriptionPackage) => {
    ...
    if (result.success && !result.pending) {
      ...
      Alert.alert('Payment flow started', 'Complete the payment in the browser and return to TracMeds to unlock family sharing.');
          ...
          {loadingOfferings ? (
            ...
            <ScrollView style={{ maxHeight: 320 }}>
              {sorted.map(pkg => (
                <TouchableOpacity
                  key={pkg.identifier}
                  style={[s.planCard, pkg.packageType === 'ANNUAL' && { borderColor: colors.primary, borderWidth: 2 }]}
