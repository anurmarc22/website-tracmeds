...
function getCheckoutUrl(pkg: SubscriptionPackage) {
  ...
  // Use the website entrypoint that now serves the purchase page.
...
export function SubscriptionProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const handleDeepLink = async (url: string | null) => {
      ...
      const isSuccess = normalized.includes('success') || normalized.includes('paid') || normalized.includes('razorpay_payment_id=') || normalized.includes('status=success');
      if (isSuccess) {
        try {
